...
public class CleaningPanel extends JPanel
{
	...
	WorldCell[][] world;
	private HashMap<String, Pair<Integer, Integer>> locations;
		...
		if (world != null)
		{
			...
			for (int x = 0; x < world[0].length; x++)
			{
				for (int y = 0; y < world.length; y++)
				{
					...
					g.setColor(Color.black);
					g.drawString("Room:" + world[y][x].getZoneID(), 2+(x * widthOfCell), (g.getFontMetrics().getHeight() * 2) + (y * heightOfCell));
				}
			}
			...
			for (String ag : locations.keySet())
			{
				...
				g.setColor(Color.BLACK);
				g.drawString(ag, tx, ty + (heightOfCell / 2));

			}
		}
	}

	public void setWorld(WorldCell[][] world2, HashMap<String, Pair<Integer, Integer>> agentLocations,
